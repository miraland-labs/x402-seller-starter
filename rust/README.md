...
This is a teaching/reference crate — no fees, no balance product logic — suitable to link from the [x402 hub](https://github.com/miraland-labs/x402). Sibling starters for [TypeScript](../typescript/README.md) and [Python](../python/README.md) share the same env vars and wire shape.

**Scope.** This starter targets the **`exact`** (UniversalSettle) rail — instant micro-payments. For the **`sla-escrow`** rail (long-running, oracle-witnessed delivery), see the [pr402 SLA docs](https://ipay.sh/onboarding_guide.md) and pr402's own examples; `find_payto` in this repo is exact-only.
...
4. **Send proof to the seller** — retry the same HTTP request with header **`PAYMENT-SIGNATURE:`** set to the JSON body you would send to facilitator **verify** (signed tx inside `paymentPayload`). Raw UTF-8 JSON is the interop default; base64-of-that-JSON is also accepted by this starter and by pr402.
